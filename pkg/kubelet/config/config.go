...
func NewPodConfig(mode PodConfigNotificationMode, recorder toolsevents.EventRecorder) *PodConfig {
...
func newPodStorage(updates chan<- kubetypes.PodUpdate, mode PodConfigNotificationMode, recorder toolsevents.EventRecorder) *podStorage {
	return &podStorage{
...
func filterInvalidPods(pods []*v1.Pod, source string, recorder toolsevents.EventRecorder) (filtered []*v1.Pod) {
	...
	for i, pod := range pods {
		...
		if names.Has(name) {
			...
			recorder.Eventf(pod, nil, v1.EventTypeWarning, events.FailedValidation, "Validating", "Error validating pod %s from %s due to duplicate pod name %q, ignoring", format.Pod(pod), source, pod.Name)
			continue
		} else {
			names.Insert(name)
		}
