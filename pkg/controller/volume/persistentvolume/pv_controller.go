...
func (ctrl *PersistentVolumeController) emitEventForUnboundDelayBindingClaim(claim *v1.PersistentVolumeClaim) error {
	...
	ctrl.eventRecorder.Eventf(claim, nil, v1.EventTypeNormal, reason, "Waiting", message)
	return nil
}
	...
	if claim.Spec.VolumeName == "" {
		...
		if volume == nil {
			...
			case storagehelpers.GetPersistentVolumeClaimClass(claim) != "":
				...
				return nil
			default:
				ctrl.eventRecorder.Eventf(claim, nil, v1.EventTypeNormal, events.FailedBinding, "Binding", "no persistent volumes available for this claim and no storage class is set")
			}
		...
		if !found {
			...
			if volume.Spec.ClaimRef == nil {
				...
				if err = checkVolumeSatisfyClaim(volume, claim); err != nil {
					...
					ctrl.eventRecorder.Eventf(claim, nil, v1.EventTypeWarning, events.VolumeMismatch, "Binding", msg)
				...
				if !metav1.HasAnnotation(claim.ObjectMeta, pvutil.AnnBoundByController) {
					...
					ctrl.eventRecorder.Eventf(claim, nil, v1.EventTypeWarning, events.FailedBinding, "Binding", claimMsg)
	...
	if volume.Spec.ClaimRef == nil {
		...
		if claim == nil {
			...
			if pvutil.CheckVolumeModeMismatches(&claim.Spec, &volume.Spec) {
				...
				ctrl.eventRecorder.Eventf(volume, claim, v1.EventTypeWarning, events.VolumeMismatch, "Binding", volumeMsg)
				...
				ctrl.eventRecorder.Eventf(claim, volume, v1.EventTypeWarning, events.VolumeMismatch, "Binding", claimMsg)
				// Skipping syncClaim
				return nil
			}
...
func (ctrl *PersistentVolumeController) updateClaimStatusWithEvent(claim *v1.PersistentVolumeClaim, phase v1.PersistentVolumeClaimPhase, volume *v1.PersistentVolume, eventtype, reason, message string) (*v1.PersistentVolumeClaim, error) {
	...
	ctrl.eventRecorder.Eventf(newClaim, nil, eventtype, reason, "Updating", message)
...
func (ctrl *PersistentVolumeController) updateVolumePhaseWithEvent(volume *v1.PersistentVolume, phase v1.PersistentVolumePhase, eventtype, reason, message string) (*v1.PersistentVolume, error) {
	...
	ctrl.eventRecorder.Eventf(newVol, nil, eventtype, reason, "Updating", message)
...
func (ctrl *PersistentVolumeController) recycleVolumeOperation(volume *v1.PersistentVolume) {
	...
	if used && !claimCached {
		...
		ctrl.eventRecorder.Eventf(volume, nil, v1.EventTypeNormal, events.VolumeFailedRecycle, "Recycling", msg)
		return
	}
	...
	// Send an event
	ctrl.eventRecorder.Eventf(volume, nil, v1.EventTypeNormal, events.VolumeRecycled, "Recycling", "Volume recycled")
...
func (ctrl *PersistentVolumeController) deleteVolumeOperation(volume *v1.PersistentVolume) (string, error) {
	...
	if err != nil {
		...
		if volerr.IsDeletedVolumeInUse(err) {
			...
			ctrl.eventRecorder.Eventf(volume, nil, v1.EventTypeNormal, events.VolumeDelete, "Deleting", err.Error())
		} else {
...
func (ctrl *PersistentVolumeController) newRecyclerEventRecorder(volume *v1.PersistentVolume) recyclerclient.RecycleEventRecorder {
	return func(eventtype, message string) {
		ctrl.eventRecorder.Eventf(volume, nil, eventtype, events.RecyclerPod, "Recycling", "Recycler pod: %s", message)
	}
}
