...
func (rc *reconciler) reportMultiAttachError(volumeToAttach cache.VolumeToAttach, nodes []types.NodeName) {
	...
	if len(pods) == 0 {
		...
		for _, pod := range volumeToAttach.ScheduledPods {
			rc.recorder.Eventf(pod, nil, v1.EventTypeWarning, kevents.FailedAttachVolume, "Attaching Volume", simpleMsg)
		}
	...
	for _, scheduledPod := range volumeToAttach.ScheduledPods {
		...
		rc.recorder.Eventf(scheduledPod, nil, v1.EventTypeWarning, kevents.FailedAttachVolume, "Attaching Volume", simpleMsg)
	}
