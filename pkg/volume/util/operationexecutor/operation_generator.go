...
func (og *operationGenerator) GenerateAttachVolumeFunc(
	...
	attachVolumeFunc := func() volumetypes.OperationContext {
		...
		for _, pod := range volumeToAttach.ScheduledPods {
			og.recorder.Eventf(pod, nil, v1.EventTypeNormal, kevents.SuccessfulAttachVolume, "Attaching Volume", simpleMsg)
		}
	...
	eventRecorderFunc := func(err *error) {
		if *err != nil {
			for _, pod := range volumeToAttach.ScheduledPods {
				og.recorder.Eventf(pod, nil, v1.EventTypeWarning, kevents.FailedAttachVolume, "Attaching Volume", (*err).Error())
			}
		}
	}
...
func (og *operationGenerator) GenerateMountVolumeFunc(
	...
	eventRecorderFunc := func(err *error) {
		if *err != nil {
			og.recorder.Eventf(volumeToMount.Pod, nil, v1.EventTypeWarning, kevents.FailedMountVolume, "Mounting Volume", (*err).Error())
		}
	}
...
func (og *operationGenerator) checkForFailedMount(volumeToMount VolumeToMount, mountError error) {
	...
	if volumetypes.IsFilesystemMismatchError(mountError) {
		...
		og.recorder.Eventf(pv, nil, v1.EventTypeWarning, kevents.FailedMountOnFilesystemMismatch, "Mounting Volume", simpleMsg)
	}
}
...
func (og *operationGenerator) GenerateMapVolumeFunc(
	...
	if affinityErr != nil {
		...
		og.recorder.Eventf(volumeToMount.Pod, nil, v1.EventTypeWarning, kevents.FailedMountVolume, "Mounting Volume", eventErr.Error())
	...
	if newMapperErr != nil {
		...
		og.recorder.Eventf(volumeToMount.Pod, nil, v1.EventTypeWarning, kevents.FailedMapVolume, "Mapping Volume", eventErr.Error())
		return volumetypes.GeneratedOperations{}, detailedErr
	}
	...
	mapVolumeFunc := func() (operationContext volumetypes.OperationContext) {
		...
		verbosity := klog.Level(4)
		og.recorder.Eventf(volumeToMount.Pod, nil, v1.EventTypeNormal, kevents.SuccessfulMountVolume, "Mounting Volume", simpleMsg)
		klog.V(verbosity).Infof(detailedMsg)
	...
	eventRecorderFunc := func(err *error) {
		if *err != nil {
			og.recorder.Eventf(volumeToMount.Pod, nil, v1.EventTypeWarning, kevents.FailedMapVolume, "Mapping Volume", (*err).Error())
		}
	}
...
func (og *operationGenerator) GenerateExpandVolumeFunc(
	...
	expandVolumeFunc := func() volumetypes.OperationContext {
		...
		if !volumePlugin.RequiresFSResize() || !fsVolume {
			...
			og.recorder.Eventf(pvc, nil, v1.EventTypeNormal, kevents.VolumeResizeSuccess, "Expanding Volume", successMsg)
		} else {
	...
	eventRecorderFunc := func(err *error) {
		if *err != nil {
			og.recorder.Eventf(pvc, nil, v1.EventTypeWarning, kevents.VolumeResizeFailed, "Expanding Volume", (*err).Error())
		}
	}
...
func (og *operationGenerator) GenerateExpandInUseVolumeFunc(
	...
	eventRecorderFunc := func(err *error) {
		if *err != nil {
			og.recorder.Eventf(volumeToMount.Pod, nil, v1.EventTypeWarning, kevents.VolumeResizeFailed, "Expanding InUseVolume", (*err).Error())
		}
	}
	...
	if expandableVolumePlugin != nil &&
		...
		if pvcStatusCap.Cmp(pvSpecCap) < 0 {
			...
			if volumeToMount.VolumeSpec.ReadOnly {
				...
				og.recorder.Eventf(volumeToMount.Pod, pvc, v1.EventTypeWarning, kevents.FileSystemResizeFailed, "Expanding Volume", simpleMsg)
				og.recorder.Eventf(pvc, volumeToMount.Pod, v1.EventTypeWarning, kevents.FileSystemResizeFailed, "Expanding Volume", simpleMsg)
				return true, nil
			}
			...
			og.recorder.Eventf(volumeToMount.Pod, pvc, v1.EventTypeNormal, kevents.FileSystemResizeSuccess, "Expanding Volume", simpleMsg)
			og.recorder.Eventf(pvc, volumeToMount.Pod, v1.EventTypeNormal, kevents.FileSystemResizeSuccess, "Expanding Volume", simpleMsg)
